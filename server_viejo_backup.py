...
from router import necesita_busqueda, es_confirmacion, es_cancelacion
from agente_jarvis import preguntar_jarvis
...
def chat():
    ...
    try:
        ...
        # --- Router Jarvis (local, sin limites) / Atlas (Gemini+busqueda) ---
        pendiente_atlas = session.get('jarvis_atlas_pendiente')
        fue_confirmacion_atlas = False
        if pendiente_atlas and not imagen_base64 and not documento_base64:
            if es_confirmacion(mensaje_usuario):
                session['jarvis_atlas_pendiente'] = None
                mensaje_usuario = pendiente_atlas
                fue_confirmacion_atlas = True
            elif es_cancelacion(mensaje_usuario):
                session['jarvis_atlas_pendiente'] = None
                respuesta_cancelada = 'Entendido, señor. No realizo la busqueda.'
                historial.append({'role': 'user', 'texto': mensaje_usuario})
                historial.append({'role': 'assistant', 'texto': respuesta_cancelada})
                guardar_memoria(usuario_id, historial)
                return jsonify({'respuesta': respuesta_cancelada, 'mostrar_pdf': False})
            else:
                session['jarvis_atlas_pendiente'] = None

        if not fue_confirmacion_atlas and not imagen_base64 and not documento_base64 and necesita_busqueda(mensaje_usuario):
            session['jarvis_atlas_pendiente'] = mensaje_usuario
            respuesta_pregunta = 'Necesito comunicarme con Atlas para consultar eso, señor. ¿Procedo con la busqueda?'
            historial.append({'role': 'user', 'texto': mensaje_usuario})
            historial.append({'role': 'assistant', 'texto': respuesta_pregunta})
            guardar_memoria(usuario_id, historial)
            return jsonify({'respuesta': respuesta_pregunta, 'mostrar_pdf': False})

        if not fue_confirmacion_atlas and not imagen_base64 and not documento_base64:
            historial_openai = [
                {'role': 'user' if h.get('role') == 'user' else 'assistant', 'content': h.get('texto', '')}
                for h in historial[-10:]
            ]
            respuesta_local = preguntar_jarvis(mensaje_usuario, historial_openai)
            historial.append({'role': 'user', 'texto': mensaje_usuario})
            historial.append({'role': 'assistant', 'texto': respuesta_local})
            guardar_memoria(usuario_id, historial)
            return jsonify({'respuesta': respuesta_local, 'mostrar_pdf': False})
        # --- fin router ---
